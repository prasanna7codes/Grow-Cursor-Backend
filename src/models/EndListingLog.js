...
const EndListingLogSchema = new mongoose.Schema({
  ...
  // duplicate_sku/expiry_listing/sku_listing_manager/keyword_end_listing
  // sources, which aren't run-scoped).
  run: { type: mongoose.Schema.Types.ObjectId, ref: 'AmazonStockCheckRun', default: null, index: true },
  source: {
    type: String,
    enum: ['duplicate_sku', 'expiry_listing', 'amazon_stock_check', 'sku_listing_manager', 'keyword_end_listing'],
    required: true,
  },
